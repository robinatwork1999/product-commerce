...
const CardInfo = ({ cardText, cardSubText, cardIcon, cardType, type, modalData }: CardInfoTypes): JSX.Element => {
    const [isModalOpen, setModal] = useState<boolean>(false);
    return (
        <React.Fragment>
            <FadeInSection type={cardType}>
                <div className={`card card-${cardType}`} role="button" onClick={() => setModal(true)}>
                    <div className="card-info">
                        <div className="card-info__icon">{cardIcon && <Image src={`${cardIcon}`} alt={''} width={200} height={200} />}</div>
            ...
            <ModalComponent isOpen={isModalOpen} setModal={setModal}>
                {type === 'voice' ? <VoiceAssistant {...modalData} /> : <ObjectDetection {...modalData} />}
            </ModalComponent>
        </React.Fragment>
    );
};
